...
fastify.post('/api/agent/chat', async (request, reply) => {
    ...
    // 设置标准的 Server-Sent Events (SSE) 响应头
    ...
    const config = {
        configurable: {
            thread_id: threadId || `scau_thread_${Date.now()}`
        }
    };

    try {
        // 使用 graphApp.astream 进行异步流式输出
        // streamMode: 'updates' 返回每个节点的增量更新，格式：{ [节点名]: { 节点输出 } }
        const streamResult = await graphApp.astream(
            { query },
            {
                ...config,
                streamMode: 'updates'
        ...
        for await (const chunk of streamResult) {
            ...
            executionSpan.span({
                name: nodeName,
                input: { query: nodeUpdate.queryRewrite || query },
                output: {
                    currentDraft: nodeUpdate.currentDraft,
        ...
        // 更新 Trace 和 Span 状态
        executionSpan.update({
            output: { completed: true, nodeCount, nodes: nodeOutputs },
