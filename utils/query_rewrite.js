 ...
 * @param {Object} config - LangGraph config（用于获取 trace）
...
export async function rewriteQuery(query, chatHistory = [], config = {}) {
    ...
    try {
        const message = await chatClient.create([
            {
                role: "system",
                content: `你是专业的 RAG 检索优化助手。你的任务是将用户的口语化问题改写成更适合向量数据库检索的精准关键词。
...
示例：
输入："那个做 AI 大模型的公司叫什么来着"
输出："AI 大模型公司 名称 商汤科技 旷视科技"

输入："怎么让 RAG 的回答更准啊"
输出："RAG 检索精度 优化策略 准确率提升"`
            },
            {
                role: "user",
        ...
        const rewrittenQuery = message.content?.trim() || message;
